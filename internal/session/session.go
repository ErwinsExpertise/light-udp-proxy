...
func (t *Table) evictExpired() {
	...
	// callbacks is reused across shards to avoid repeated allocations.
	// It is reset to length 0 (but keeps capacity) before each shard sweep.
	var callbacks []func()
	for i := range t.shards {
		s := &t.shards[i]
		...
		for k, sess := range s.entries {
			...
			if sess.LastSeenTime().Before(cutoff) {
				delete(s.entries, k)
				t.count.Add(-1)
				// Collect callbacks; invoke them after the lock is released to
				// avoid holding the shard lock during callback execution and to
				// prevent a goroutine burst when many sessions expire at once.
				callbacks = append(callbacks, sess.callEvict)
			}
		}
		s.mu.Unlock()
